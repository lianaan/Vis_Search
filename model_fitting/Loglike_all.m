function LL = Loglike_all(pars, data,mi, type)

%type=1 -> Det
%type=2 -> Loc
%type=3 -> Det & Loc joint

Nsamp = 1200;%2000;%3000;%2000;
...
tau   = exp(pars(5)); % scale of the gamma distribution
...
if (type==1) | (type==3)
    ...
    for Nind = 1:length(Nvec_d)
        ...
        stims     = repmat(sdata_de,[1,1, Nsamp]); % Read in data and replicate
    ...
    LL_d=nan(Ntrials_d,1);
    LL_d(Resp_d==1)=log(ptp(Resp_d==1));
    LL_d(Resp_d==0)=log(1-ptp(Resp_d==0));
...
if (type==2) | (type==3)
    ...
    for Nind = 1:length(Nvec_l)
        ...
        if (mi == 1) 
            ...
            plhat(idx) = prob_l(sub2ind(size(prob_l), [1:1:length(idx)]',Resp_le(idx)));
